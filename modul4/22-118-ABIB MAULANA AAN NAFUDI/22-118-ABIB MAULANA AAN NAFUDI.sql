...
-- Menampilkan Data Diurutkan (ASC, DESC) dan Klausa Lainnya
-- Berikut adalah beberapa contoh perintah SQL yang menggunakan klausa ORDER BY, GROUP BY, HAVING, rentang tanggal, dan kriteria lainnya:

-- 1. Mengurutkan Data

-- Mengurutkan data anggota berdasarkan nama secara ascending:
SQL
SELECT * FROM anggota
ORDER BY Nama_Anggota ASC;

-- Mengurutkan data buku berdasarkan tahun terbit secara descending:
SQL
SELECT * FROM buku
ORDER BY Tahun_Buku DESC;

-- 2. Mengelompokkan Data (GROUP BY, HAVING)

-- Menghitung jumlah peminjaman per buku:
SQL
SELECT Kode_Buku, COUNT(*) AS Jumlah_Peminjaman
FROM peminjaman
GROUP BY Kode_Buku;

-- Menampilkan buku yang dipinjam lebih dari 2 kali:
SQL
SELECT Kode_Buku, COUNT(*) AS Jumlah_Peminjaman
FROM peminjaman
GROUP BY Kode_Buku
...
-- Menampilkan peminjaman yang dilakukan antara bulan September 2021 dan Januari 2024:
SQL
SELECT * FROM peminjaman
WHERE Tanggal_Pinjam BETWEEN '2021-09-01' AND '2024-01-01';

-- 4. Menampilkan Produk/Barang/Jasa Paling Banyak dan Paling Sedikit Muncul

-- Produk/barang/jasa (buku) yang paling banyak dipinjam:
SQL
SELECT Kode_Buku, COUNT(*) AS Jumlah_Peminjaman
FROM peminjaman
GROUP BY Kode_Buku
...
-- Produk/barang/jasa (buku) yang paling sedikit dipinjam:
SQL
SELECT Kode_Buku, COUNT(*) AS Jumlah_Peminjaman
FROM peminjaman
GROUP BY Kode_Buku
...
-- 5. Menampilkan Produk/Barang/Jasa Berdasarkan Huruf Awal "a"

SQL
SELECT * FROM buku
WHERE Judul_Buku LIKE 'a%';

-- 6. Mencari Konsumen/User yang Paling Banyak Melakukan Transaksi

-- Mencari ID anggota yang paling banyak melakukan peminjaman:
SQL
SELECT IdAnggota, COUNT(*) AS Jumlah_Peminjaman
FROM peminjaman
GROUP BY IdAnggota
ORDER BY Jumlah_Peminjaman DESC
LIMIT 1;

-- Mencari nama anggota yang paling banyak melakukan peminjaman:
SQL
SELECT a.Nama_Anggota, COUNT(*) AS Jumlah_Peminjaman
FROM peminjaman p
...
-- 7. Mencari Konsumen/User yang Melakukan Transaksi Lebih dari 3
...
-- 8. Menampilkan Data Konsumen/User Lengkap dengan Transaksi

Menampilkan DATA anggota beserta peminjamannya:
SQL
SELECT a.Nama_Anggota, a.Angkatan_Anggota, a.Tempat_Lahir_Anggota, a.Tanggal_Lahir_Anggota, a.No_Telp, a.Jenis_Kelamin, a.Status_Pinjam, p.Kode_Peminjaman, p.Tanggal_Pinjam, p.Tanggal_Kembali, p.Kode_Buku
FROM anggota a
JOIN peminjaman p ON a.IdAnggota = p.IdAnggota;

-- 9. Inner Join untuk Menampilkan Data Terkait
SELECT p.Kode_Peminjaman, a.Nama_Anggota, b.Judul_Buku, p.Tanggal_Pinjam, p.Tanggal_Kembali
FROM peminjaman p
